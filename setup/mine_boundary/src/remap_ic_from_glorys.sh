...
def main():
    ...
    ds=xr.open_dataset(global_file).rename({'thetao':'temp','so':'salt','zos':'ssh',
						'uo':'u','vo':'v'})
...
def interpolation_method(ds_cut,ds_ccs,fronteira):     
    ...
    if fronteira in ['north']:
        ...
        # *** 3. Subsetting the domain no need
        ds_cut = ds_cut.rename({'longitude': 'lon', 'latitude': 'lat','depth': 'lev'}).isel(time=0)        
        if view_results:
                print(ds_cut)
    ...
    print('ok') 
#========================================================================    
def transformz(ds, dsCtrl2, preffix='ds_', zcoord_in='z_l', zcoord_to='depth'):
	from xgcm import Grid
	if zcoord_to not in ds.data_vars:
              print(ds.data_vars)
              print('insert zcoord_to:');zcoord_to=input()
	for var in dsCtrl2.variables:
                if var not in dsCtrl2.dims and dsCtrl2[var].ndim > 1 and var not in ['time_bnds']:
                    print(var)
	...
	print(newds.dims)
        try:
                newds=newds.transpose('time','z_to','xh','yh').rename({'z_to':'depth','xh':'lon','yh':'lat'})
        except:
