...
def interpolation_method(ds_cut,ds_ccs,fronteira):     
    ...
    if fronteira in ['north']:
        ...
        # *** 3. Subsetting the domain no need
        try:
                ds_cut = ds_cut.rename({'xu_ocean': 'lon', 'yu_ocean': 'lat','st_ocean': 'lev'}).sel(lon=slice(290, 350), lat=slice(-42,-10)).isel(time=0)
        except:
                ds_cut = ds_cut.rename({'xt_ocean': 'lon', 'yt_ocean': 'lat','st_ocean': 'lev'}).sel(lon=slice(290, 350), lat=slice(-42,-10)).isel(time=0)        
        if view_results:
                print(ds_cut)
