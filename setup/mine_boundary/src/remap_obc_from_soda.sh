#!/home/nicole/miniconda3/envs/obcic/bin/python
...
def main():
    ...
    if(len(sys.argv)<1):
	    print(f"uso: {sys.argv[0]} <arquivo_hgrid> <global_file> <var>")
	    print(f'fornecido: {sys.argv} \n\n', '------\t'*10)
	    quit()
    else:
	    print('.............Generating BC anc IC from global data.............') 
	    print(f' {sys.argv[:]}')   
    use_location=True
    undef=1e20
    global sel_var, dir_out, dz
    dir_out='./'


    if "ocean_hgrid" in sys.argv[1] :         
        #regional_grid=open_grid(sys.argv[1]) # com ocean_hgrid.nc sem supergrid
    ...
    try:
        lon, lat, lev, time = ds.indexes.values()  
    except:
        try:
                time, xt, yt, st, sw = ds.indexes.values()  
        except:
                print(f'ERRO! arquivo não tem as mesmas coords do soda, arrumar o script {sys.argv[0]}')
                print("disponível:", ds.indexes.values())
                quit()
    print(f" coords={ds.dims}"  )
    print(f" coords={regional_grid.dims}"  )        
    ...
    interpolation_method(ds,regional_grid,fronteira)
...
def interpolation_method(ds_cut,ds_ccs,fronteira):     
    ...
    global drowned_var_east, drowned_var_north, drowned_var_south, var_north, var_south, var_east, var_ic_ccs
    ...
    if fronteira in ['north']:
        ...
        regrid_domain = xesmf.Regridder(ds_cut.rename({'lon': 'lon', 'lat': 'lat'}), 
                                ds_ccs.rename({'x': 'lon', 'y': 'lat'}),
                                'bilinear', periodic=False, #reuse_weights=True, 
                                 filename='regrid_domain.nc')

    if sel_var in ['SSH','ssh']:
        drowned_var = flood_kara(ds_cut[sel_var], xdim='lon', ydim='lat', tdim='time')
        if fronteira in ['north']:
                var_north = regrid_north(ds_cut[sel_var])
                drowned_var_north = var_north.bfill(dim='locations').ffill(dim='locations')
        if fronteira in ['south']:        
                var_south = regrid_south(ds_cut[sel_var])
                drowned_var_south = var_south.bfill(dim='locations').ffill(dim='locations')
        if fronteira in ['east']:        
                var_east = regrid_east(ds_cut[sel_var])
                drowned_var_east = var_east.bfill(dim='locations').ffill(dim='locations')
    else:    
        drowned_var = flood_kara(ds_cut[sel_var], xdim='lon', ydim='lat', zdim='lev', tdim='time')
    ...
    if sel_var not in ['SSH','ssh']:
        if fronteira in ['north']:
                var_north = regrid_north(ds_cut[sel_var])
                drowned_var_north = var_north.bfill(dim='locations').ffill(dim='locations').ffill(dim='lev').bfill(dim='lev')
        if fronteira in ['south']:            
                var_south = regrid_south(ds_cut[sel_var])
                drowned_var_south = var_south.bfill(dim='locations').ffill(dim='locations').ffill(dim='lev').bfill(dim='lev')
        if fronteira in ['east']:         
                var_east = regrid_east(ds_cut[sel_var])
                drowned_var_east = var_east.bfill(dim='locations').ffill(dim='locations').ffill(dim='lev').bfill(dim='lev')

      
    if fronteira in ['initial']:             
        alldrowned_var = drowned_var.ffill(dim='lev').bfill(dim='lev')
        var_ic_ccs = regrid_domain(alldrowned_var)
...
def write_obc(da, dadz, varname, fname='obc_teste.nc', fill_value=1e20):
    ...
    if 'ssh' not in varname:
        ds_.lev.attrs={'axis' : "Z", 'cartesian_axis' : "Z",
                       'positive' : "up", 'units' : "km m-3", 'long_name' : "Layer Target Potential Density"}
...
#                               LOCATIONS 
        ...
        for pr in params:
            print(pr)
            print(f"{pr['tr_in'].coords}")
            print(f"{pr['tr_in'].lat} {pr['tr_in'].locations}")  

#            pr['tr_in'].isel(time=0).plot()
#            plt.show()                  

            # exporting coords         
            try:
                #time, lev, loc = ds.indexes.values() 
                time=pr['tr_in'].time
                lev=pr['tr_in'].lev
                lat=pr['tr_in'].lat
                lon=pr['tr_in'].lon            
...
#========================================================================
            ...
            da_dz=xr.DataArray(dz,coords=[('time',time),('lev',lev),
                        ('lat',lat),('lon',lon)])
...
#            ds_.time.attrs={"units":'days since 1900-01-01 00:00:00',"calendar":"365_day"}
            ...
            print(f'>Use locations {use_location} netcdf not saved')
            quit()
