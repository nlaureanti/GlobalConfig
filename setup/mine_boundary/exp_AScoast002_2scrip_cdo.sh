...
dir_saida='output_exp_AScoast002_icpython/'

# Regional model definitions
fgrid='/home/nicole/Documentos/INPE/mom/exps_mom6//exp_AScoast_cdo/workdir/INPUT/ocean_hgrid.nc'
lati='-40'
latf='-20'
loni='300'
lonf='340'
...
method='soda' #options: depth, h, soda
dzfile='/home/nicole/mom/create_layer/com_depth/create_dz_to_obcs_layer.nc' #use depth
dzfile_h='/home/nicole/mom/create_obc/create_obc_mix/dz_prog.nc' #use h
################################################################################
################################################################################
if [[ ${#1} == 0 ]] ;then 
read -p "remap_obc_cdo? 1-yes 0-no    " remap_cdo
else
remap_cdo=$1
fi

if [[ ${#2} == 0 ]] ;then 
read -p "remap_obc_python? 1-yes 0-no    " remap_python
else
remap_python=$2
fi
...
for n in $(seq 0 $(( ${#vvars[@]} -1 )) ); do
	...
	#${src}/fill_OBC_horz.py tmp_ic_${v}.nc tmp2_ic_${v}.nc 
	#if [[ $v != 'ssh' && ! -f ic_h.nc ]] ; then 
	#        cdo -s -L -remapdis,$(basename ${fgrid})  $dzfile tmp2_ic_h.nc 
